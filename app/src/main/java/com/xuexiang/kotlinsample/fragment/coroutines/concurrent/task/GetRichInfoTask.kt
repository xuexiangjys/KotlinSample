...
import com.xuexiang.kotlinsample.fragment.coroutines.concurrent.processor.GetRichInfoProcessor
 ...
 */
class GetRichInfoTask(logger: LoggerTextView?) : AbstractTask(logger) {
    @Throws(Exception::class)
    override fun doTask() {
        val product: Product? = taskParam.getObject(ProductTaskConstants.KEY_PRODUCT, Product::class.java)
        if (product == null || StringUtils.isEmpty(product.richId)) {
            notifyTaskFailed(ITaskResult.ERROR, "product is null or richId is empty!")
            return
        }
        GetRichInfoProcessor(mLogger, product.richId!!)
            .setProcessorCallback(object : AbstractProcessor.IProcessorCallback<RichInfo?> {
                override fun onSuccess(richInfo: RichInfo?) {
                    taskParam.put(ProductTaskConstants.KEY_PRODUCT, product.apply { rich = richInfo })
                    notifyTaskSucceed()
                }
