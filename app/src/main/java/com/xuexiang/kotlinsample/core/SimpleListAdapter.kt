 ...
 */
class SimpleListAdapter(context: Context?, data: List<Map<String?, String?>?>?) : BaseListAdapter<Map<String?, String?>, SimpleListAdapter.ViewHolder>(context, data) {
    ...
    override fun convert(holder: ViewHolder, item: Map<String?, String?>, position: Int) {
        holder.mTvTitle!!.text = item[KEY_TITLE]
        if (!StringUtils.isEmpty(item[KEY_SUB_TITLE])) {
            holder.mTvSubTitle!!.text = item[KEY_SUB_TITLE]
